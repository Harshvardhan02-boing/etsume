...
data object MangaLibraryTab : Tab {
    ...
    override fun Content() {
        ...
        val visibleItemsForPage: (Int) -> List<MangaLibraryItem> = { page ->
            state.getLibraryItemsByPage(page).filter { item ->
                !isBlockedExplicitContent(
                    showNsfw = showNsfw,
                    sourceId = item.libraryManga.manga.source,
                    mangaExtensionManager = mangaExtensionManager,
                    fields = buildList {
                        add(item.libraryManga.manga.title)
                        add(item.libraryManga.manga.description)
                        addAll(item.libraryManga.manga.genre.orEmpty())
                    },
                )
            }
        }
        val hasVisibleLibraryItems = state.categories.indices.any { visibleItemsForPage(it).isNotEmpty() }
        ...
        Scaffold(
            ...
            Box(modifier = Modifier.fillMaxSize()) {
                Column(modifier = Modifier.padding(top = contentPadding.calculateTopPadding())) {
                    LibraryTab.MediaSwitcher(
                        selectedPane = LibraryTab.LibraryPane.Manga,
                        modifier = Modifier.padding(horizontal = 16.dp, vertical = 12.dp),
                    )
                    when {
                        state.isLoading -> LoadingScreen(Modifier.padding(PaddingValues(bottom = contentPadding.calculateBottomPadding())))
                        state.searchQuery.isNullOrEmpty() && !state.hasActiveFilters && !hasVisibleLibraryItems -> {
                            val handler = LocalUriHandler.current
                            EmptyScreen(
                                stringRes = MR.strings.information_empty_library,
                                modifier = Modifier.padding(PaddingValues(bottom = contentPadding.calculateBottomPadding())),
                            ...
                            MangaLibraryContent(
                                ...
                                getNumberOfMangaForCategory = { category ->
                                    state.categories.indexOf(category)
                                        .takeIf { it >= 0 }
                                        ?.let { visibleItemsForPage(it).size }
                                },
