...
class PlayerActivity : BaseActivity() {
    ...
    private lateinit var playerBackCallback: OnBackPressedCallback
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        playerBackCallback = object : OnBackPressedCallback(true) {
            override fun handleOnBackPressed() {
                handlePlayerBackPress()
            }
        }
        onBackPressedDispatcher.addCallback(this, playerBackCallback)
        binding.root.post { applyPlayerLayoutForOrientation(resources.configuration.orientation) }
        ...
        binding.controls.setContent {
            TachiyomiTheme {
                PlayerControls(
                    viewModel = viewModel,
                    onBackPress = {
                        this@PlayerActivity.handlePlayerBackPress()
                    },
    ...
    private fun dispatchDefaultBackPress() {
        if (::playerBackCallback.isInitialized) {
            playerBackCallback.isEnabled = false
            onBackPressedDispatcher.onBackPressed()
            playerBackCallback.isEnabled = true
        } else {
            finish()
        }
    }

    internal fun handlePlayerBackPress() {
    ...
    override fun onResume() {
        if (!player.isExiting) {
            super.onResume()
            refreshSystemUiForCurrentOrientation()
            return
        }
    ...
    override fun onConfigurationChanged(newConfig: Configuration) {
        if (!isInPictureInPictureMode) {
            if (newConfig.orientation == Configuration.ORIENTATION_LANDSCAPE) {
                ...
                viewModel.applyPortraitLayoutAspect()
            }
        } else {
            ...
            pipReceiver = object : BroadcastReceiver() {
                override fun onReceive(context: Context?, intent: Intent?) {
                    ...
                    when (intent.getIntExtra(PIP_INTENT_ACTION, 0)) {
                        ...
                        PIP_REWIND -> viewModel.seekBy(-10)
                    }
                    setPictureInPictureParams(createPipParams())
                }
            }
    ...
    private fun setupPlayerOrientation() {
        ...
        requestedOrientation = when (playerPreferences.defaultPlayerOrientationType().get()) {
            PlayerOrientation.Free -> ActivityInfo.SCREEN_ORIENTATION_PORTRAIT
            PlayerOrientation.Video -> ActivityInfo.SCREEN_ORIENTATION_PORTRAIT
            PlayerOrientation.Portrait -> ActivityInfo.SCREEN_ORIENTATION_PORTRAIT
            PlayerOrientation.ReversePortrait -> ActivityInfo.SCREEN_ORIENTATION_REVERSE_PORTRAIT
            PlayerOrientation.SensorPortrait -> ActivityInfo.SCREEN_ORIENTATION_PORTRAIT
    ...
    internal fun showPortraitPlayer() {
        requestedOrientation = ActivityInfo.SCREEN_ORIENTATION_PORTRAIT
        refreshSystemUiForCurrentOrientation()
    }

    internal fun showLandscapePlayer() {
        requestedOrientation = ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE
        refreshSystemUiForCurrentOrientation()
    }

    internal fun enterPipIfPossible() {
        if (isPipSupportedAndEnabled && player.paused == false) {
            enterPictureInPictureMode(createPipParams())
        }
    }

    private fun applyPlayerLayoutForOrientation(orientation: Int) {
        val playerLayoutParams = binding.player.layoutParams
        val controlsLayoutParams = binding.controls.layoutParams
        val displayMetrics = resources.displayMetrics
        val statusBarInset = currentStatusBarInset()

        playerLayoutParams.width = ViewGroup.LayoutParams.MATCH_PARENT
        controlsLayoutParams.width = ViewGroup.LayoutParams.MATCH_PARENT
        controlsLayoutParams.height = ViewGroup.LayoutParams.MATCH_PARENT

        playerLayoutParams.height = if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
            ViewGroup.LayoutParams.MATCH_PARENT
        } else {
            val targetHeight = (displayMetrics.widthPixels * 9f / 16f).roundToInt()
            val maxHeight = (displayMetrics.heightPixels * 0.44f).roundToInt()
            val minHeight = (displayMetrics.density * 220).roundToInt()
            targetHeight.coerceIn(minHeight, maxHeight)
        }

        binding.root.setBackgroundColor(
            ContextCompat.getColor(
                this,
                if (orientation == Configuration.ORIENTATION_LANDSCAPE) android.R.color.black else eu.kanade.tachiyomi.R.color.tachiyomi_background,
            ),
        )
        binding.player.translationY = 0f
        updateSystemUiForOrientation(orientation)
        binding.player.layoutParams = playerLayoutParams
        binding.controls.layoutParams = controlsLayoutParams
    ...
    internal fun currentStatusBarInset(): Int {
        val insets = window.decorView.rootWindowInsets
            ?.let { WindowInsetsCompat.toWindowInsetsCompat(it, window.decorView) }
            ?.getInsets(WindowInsetsCompat.Type.statusBars())
            ?.top
            ?: 0
        if (insets > 0) return insets

        val resourceId = resources.getIdentifier("status_bar_height", "dimen", "android")
        return if (resourceId != 0) resources.getDimensionPixelSize(resourceId) else 0
    }

    internal fun refreshSystemUiForCurrentOrientation() {
        updateSystemUiForOrientation(resources.configuration.orientation)
    }
    ...
    private fun updateSystemUiForOrientation(orientation: Int) {
        ...
        if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
            ...
            window.statusBarColor = ContextCompat.getColor(this, eu.kanade.tachiyomi.R.color.tachiyomi_background)
