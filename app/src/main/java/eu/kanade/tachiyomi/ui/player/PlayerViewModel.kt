...
class PlayerViewModel @JvmOverloads constructor(
    ...
    fun showControls() {
        ...
        if (showStatusBar || activity.resources.configuration.orientation != Configuration.ORIENTATION_LANDSCAPE) {
    ...
    fun hideControls() {
        if (activity.resources.configuration.orientation == Configuration.ORIENTATION_LANDSCAPE && !showStatusBar) {
    ...
    fun cycleScreenRotations() {
        when (activity.resources.configuration.orientation) {
            Configuration.ORIENTATION_LANDSCAPE -> activity.showPortraitPlayer()
            else -> activity.showLandscapePlayer()
        }
    ...
    fun toggleLibrary() {
        ...
        viewModelScope.launchIO {
            val added = !anime.favorite
            if (updateAnime.awaitUpdateFavorite(anime.id, added)) {
                _currentAnime.update { getAnime.await(anime.id) }
                playerUpdate.update {
                    PlayerUpdates.ShowText(
                        activity.getString(
                            if (added) {
                                eu.kanade.tachiyomi.R.string.etsume_player_added_to_library
                            } else {
                                eu.kanade.tachiyomi.R.string.etsume_player_removed_from_library
                            },
                        ),
                    )
                }
    ...
    fun downloadCurrentEpisode() {
        ...
        viewModelScope.launchIO {
            downloadManager.downloadEpisodes(anime, listOf(episode))
            playerUpdate.update {
                PlayerUpdates.ShowText(
                    activity.getString(eu.kanade.tachiyomi.R.string.etsume_player_download_queued),
                )
            }
        }
    }
