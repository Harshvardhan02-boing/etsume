...
data object AnimeLibraryTab : Tab {
    ...
    override fun Content() {
        ...
        val visibleItemsForPage: (Int) -> List<AnimeLibraryItem> = { page ->
            state.getAnimelibItemsByPage(page).filter { item ->
                !isBlockedExplicitContent(
                    showNsfw = showNsfw,
                    sourceId = item.libraryAnime.anime.source,
                    animeExtensionManager = animeExtensionManager,
                    fields = buildList {
                        add(item.libraryAnime.anime.title)
                        add(item.libraryAnime.anime.description)
                        addAll(item.libraryAnime.anime.genre.orEmpty())
                    },
                )
            }
        }
            ...
            Box(modifier = Modifier.fillMaxSize()) {
                Column(modifier = Modifier.padding(top = contentPadding.calculateTopPadding())) {
                    LibraryTab.MediaSwitcher(
                        selectedPane = LibraryTab.LibraryPane.Anime,
