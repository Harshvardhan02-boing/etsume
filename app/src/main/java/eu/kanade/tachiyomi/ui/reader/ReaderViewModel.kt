...
class ReaderViewModel @JvmOverloads constructor(
    ...
    init {
        ...
        state.map {
            Triple(
                it.viewerChapters?.currChapter?.chapter?.id,
                it.currentPage,
                it.totalPages,
            )
        }
            .distinctUntilChanged()
            .onEach { refreshChapterDrawerItems() }
            .launchIn(viewModelScope)
    }
    ...
    fun openSettingsDialog(initialPage: Int = 0) {
        mutableState.update { it.copy(dialog = Dialog.Settings(initialPage)) }
    }

    fun openCommentsDialog() {
        openSettingsDialog(initialPage = 3)
    }
    ...
    fun openChapterById(chapterId: Long) {
        ...
        if (state.value.currentChapter?.chapter?.id == chapterId) return
    ...
    data class State(
        ...
        val chapterDrawerItems: List<ChapterDrawerItem> = emptyList(),
    ...
    @Immutable
    data class ChapterDrawerItem(
        val id: Long,
        val title: String,
        val resumePage: Int?,
        val exactTotalPages: Int?,
    ...
    private fun refreshChapterDrawerItems() {
        if (manga == null || chapterId == -1L || loader == null) {
            mutableState.update { it.copy(chapterDrawerItems = emptyList()) }
            return
        }

        val currentChapterId = state.value.currentChapter?.chapter?.id
        val currentPage = state.value.currentPage
        val currentTotalPages = state.value.totalPages

        mutableState.update {
            it.copy(
                chapterDrawerItems = chapterList.map { readerChapter ->
                    val chapter = readerChapter.chapter
                    val id = chapter.id ?: -1L
                    val isCurrent = id == currentChapterId
                    val exactTotalPages = when {
                        isCurrent && currentTotalPages > 0 -> currentTotalPages
                        readerChapter.pages != null -> readerChapter.pages?.size?.takeIf { pages -> pages > 0 }
                        else -> null
                    }
                    val resumePage = when {
                        isCurrent && currentPage > 0 -> currentPage
                        chapter.last_page_read > 0 -> chapter.last_page_read + 1
                        else -> null
                    }
                    val progressFraction = when {
                        chapter.read -> 1f
                        exactTotalPages != null && exactTotalPages > 0 && resumePage != null -> {
                            resumePage.toFloat() / exactTotalPages.toFloat()
                        }
                        else -> null
                    }
                    ...
                    ChapterDrawerItem(
                        ...
                        shouldContinue = isCurrent || chapter.read || chapter.last_page_read > 0,
                        isCurrent = isCurrent,
